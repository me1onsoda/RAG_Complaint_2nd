...
async def analyze_and_store(request: ComplaintRequest):
    try:
        print(f"[*] 분석 시작 - 민원 ID: {request.complaint_id}")
        ...
        database.save_normalization(request.complaint_id, analysis, embedding)
        print(f"[+] 성공: 민원 {request.complaint_id} 데이터베이스 저장 완료")
